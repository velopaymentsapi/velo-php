...
class AcceptedPayment implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AcceptedPayment';
    ...
    public function setCurrencyType($currency_type)
    {
        if ((mb_strlen($currency_type) > 3)) {
            throw new \InvalidArgumentException('invalid length for $currency_type when calling AcceptedPayment., must be smaller than or equal to 3.');
        }
        if ((mb_strlen($currency_type) < 3)) {
            throw new \InvalidArgumentException('invalid length for $currency_type when calling AcceptedPayment., must be bigger than or equal to 3.');
        }
        if ((!preg_match("/^[A-Z]{3}$/", $currency_type))) {
            throw new \InvalidArgumentException("invalid value for $currency_type when calling AcceptedPayment., must conform to the pattern /^[A-Z]{3}$/.");
        }
